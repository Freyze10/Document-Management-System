...
class SmypcWidget(QWidget):
    def __init__(self):
        super().__init__()
        self.setup_ui()
        self.default_values()
        self.smypc_widget = None
    ...
    def setup_ui(self):
        ...
        general_info_layout.addWidget(self.delivery_date, 3, 1)
        general_info_layout.addWidget(QLabel("Manufacturing Date:"), 3, 2, Qt.AlignmentFlag.AlignRight)
        ...
        self.btn_save_as_new.setStyleSheet("""
                ...
                """)
        self.btn_save_as_new.clicked.connect(self.smypc_save_as_new_clicked)
    ...
    def default_values(self):
        self.customer_input.setText("SMYPC-SFP")
        ...
        properties_data = [
            ("Raw Material Basis", "Polymer blend", "Polymer blend", "Visual (MBPI)"),
            ("Color", "Blue", "Blue", "MBPI"),
            ("Specific Gravity", "0.9", "0.9 ± 0.20", "MBPI"),
            ("Durometer Hardness Shore \"A\"", "92.84", "90.0 ± 5.00", "ASTM D 2240"),
    ...
    def on_submit_clicked(self, print_after=False):
        ...
        if self.certified_by_name_input.text() not in self.certified_by_lists:
            window_alert.show_message(self, "Invalid Input",
                                      f"Certified By: '{self.certified_by_name_input.text()}' is not in the list.",
                                      icon_type="warning")
            return

        if print_after:
            returning_coa_id = db_con.save_pvc_free_coi(data, properties_data)
            self.open_smypc_preview(returning_coa_id, "temp")
    ...
    def load_coa_details(self, coa_id):
        ...
        try:
            ...
            if field_result:
                ...
                if field_result[8]:
                    self.manufacturing_date_input.display_value(
                        prod_date_format.dates_for_display(str(field_result[8])))
    ...
    def smypc_save_as_new_clicked(self):
        global current_coa_id
        current_coa_id = None
    ...
    def open_smypc_preview(self, coa_id, filename):
        # If the widget already exists, close it first to avoid multiple instances

        try:
            if self.smypc_widget is not None:
